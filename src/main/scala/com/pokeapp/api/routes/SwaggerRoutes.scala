...
// Rutas que sirven la documentación OpenAPI (swagger.json) y la interfaz Swagger UI.
// El spec se genera dinámicamente desde ResourceRegistry para incluir todos los recursos disponibles.
class SwaggerRoutes[F[_]: Async]:
  ...
  // Convierte un slug kebab-case ("berry-firmness") a etiqueta legible ("Berry Firmness")
  ...
  // Genera el objeto OpenAPI para un endpoint de listado paginado
  ...
  // Genera el objeto OpenAPI para un endpoint de detalle por ID o nombre
  ...
  // Spec OpenAPI 3.0 completo: combina los paths manuales de Pokémon con los generados dinámicamente
  ...
  // Sirve el spec JSON en GET /swagger/swagger.json
  ...
  // Sirve la interfaz Swagger UI en GET /swagger-ui; carga los assets desde CDN de unpkg
